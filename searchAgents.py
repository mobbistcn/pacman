...
class CornersProblem(search.SearchProblem):
  ...
  def __init__(self, startingGameState):
    ...
    self.startingGameState = startingGameState.getPacmanPosition()
    ...
    "*** YOUR CODE HERE ***"
    nextCorner = self.corners
    self.startstate = (self.startingGameState, nextCorner)
...
def cornersHeuristic(state, problem):
  ...
  for corner in corners_list:
	  distance.append(abs(p1[0]-corner[0])+abs(p1[1]-corner[1]))
	  if((0,0) in distance):
		  distance.remove((0,0))
...
def foodHeuristic(state, problem):
  ...
  for x in range(w):
    for y in range(h):
      if foodGrid[x][y]:
        distance = max(distance, abs(x-position[0]) + abs(y-position[1]))
  return distance
